...
(let ((dirname (package-desc-dir
  ...
  (copy-file "epdfinfo" (file-name-concat dirname "epdfinfo") t))
(restart-emacs)
#+END_SRC
...
(when galactron-pdf-enabled-python-server
  (galactron-default pdf-info-epdfinfo-program
                  (file-name-concat
                   (file-name-directory load-file-name) "epdfinfo")))
#+END_SRC
...
(when galactron-largefile-enabled-djvu
  (galactron-with-package-installed djvu
    (dolist (item auto-mode-alist)
...
(defun galactron--find-file (file-name)
  (interactive)
  (let ((org-file (file-name-with-extension file-name ".org")))
    (if (file-exists-p org-file)
        (find-file org-file)
      (find-file (file-name-with-extension file-name ".el")))))
    ...
    (galactron--with-messages
      (insert
       (concat
        ...
        "，部分配置未生效，"
        (buttonize "查看详情" 'galactron--find-file load-file-name)
        "\n"))))
